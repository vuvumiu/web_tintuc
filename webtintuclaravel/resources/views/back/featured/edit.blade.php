...
@section('title', 'Sửa Bài viết nổi bật')
@section('heading', 'Sửa Bài viết nổi bật')
...
<div class="col-md-12">
    <div class="card card-default">
        ...
        <form action="{{ url('admin/featured/edit/' . $item->RowID) }}" method="POST">
            @csrf
            <div class="card-body">
                ...
                <div class="form-group row">
                    ...
                    <div class="col-sm-9">
                        <select name="news_id" id="news_id" class="form-control select2" data-placeholder="Chọn bài viết nổi bật" required>
                            ...
                            @foreach($news as $n)
                                <option value="{{ $n->RowID }}" {{ old('news_id', $item->news_id) == $n->RowID ? 'selected' : '' }}>
                ...
                <div class="form-group row">
                    ...
                    <div class="col-sm-9">
                        <select name="position" id="position" class="form-control" required>
                            <option value="1" {{ old('position', $item->position) == 1 ? 'selected' : '' }}>
                                Tin chính (Hero lớn)
                            </option>
                ...
                <div class="form-group row">
                    <label class="col-sm-3 col-form-label text-right">Thứ tự</label>
                    <div class="col-sm-9">
                        <input type="number" name="Sort" value="{{ old('Sort', $item->Sort) }}"
                               class="form-control" min="0" placeholder="0">
                        <small class="form-text text-muted">Số càng nhỏ thì hiển thị càng trước.</small>
                    </div>
                </div>
                ...
                <div class="form-group row">
                    ...
                    <div class="col-sm-9">
                        <select name="Status" class="form-control">
                            <option value="1" {{ old('Status', $item->Status) == 1 ? 'selected' : '' }}>Bật</option>
                            <option value="0" {{ old('Status', $item->Status) == 0 ? 'selected' : '' }}>Tắt</option>
                        </select>
                    </div>
                </div>
